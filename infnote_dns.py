        ...
        if os.path.exists(file_infnote_db) is False:
            ...
            if response:
                ...
                logger.info('get nothing from cache,'
                            'please check the infnote_db file')
        # query response in LRUCache
        # if file_infnote_db do not change or was updating
    ...
    def handle(self):
        if not DNSServer.deq_cache.full():
            ...
            DNSServer.deq_cache.put(
                (self.request[0], self.client_address, self.request[1]))
...
class DNSServer(object):
    ...
    def start():
        # cache the request
        if(deq_size > 0):
            DNSServer.deq_cache = Queue(maxsize=deq_size)
        else:
